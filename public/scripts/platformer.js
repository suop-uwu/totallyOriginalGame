$(function () {
    ...
    function leftRightControls() {
        ...
        if (controls.right in keysDown !== controls.left in keysDown) {
            if (absoluteValue(mc.velx + currentAccel * mc.facing) <= currentMaxSpeed || absoluteValue(mc.velx + currentAccel * mc.facing) <= absoluteValue(mc.velx)) { //if the new speed is less than the max speed or it's previous speed.
